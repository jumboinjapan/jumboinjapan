...
_POI update, 2026-09-10: read AGENTS.md, then docs/poi-intake/japan-guide-operations.md. The Japan Guide reader now preserves complete evidence; an agent authors sourced facts and RU/EN copy before the first Draft / Todo POST. Live intake requires --facts. Existing drafts use copy/v2. Facts are visible in the admin UI and its authenticated API. The 10d/10e notes and POI entries in §13 below are historical audit records, not instructions for the next batch. Current progress is in docs/poi-intake/README.md._
...
_Дополнение 2026-08-26 (10e): независимый повтор полного `v3`-обхода принят — структурная дельта нулевая; 23 связи Japan Guide переведены с URL на `Source Key`. После канонизации Route Stops и L3 10e-E2 в базе 471 POI и 64 остановки, активных остановок без `POI ID` нет, остаются два осознанных `snapshot_drift`. Пять новых дочерних POI заведены через production Intake с `owner-curation:*`; полный `npm run verify` после записи прошёл. Текущие очереди и стоп-условия — `docs/poi-intake/airtable-canonicalization.md`._
...
**Канонизация базы — входное условие, а не уборка потом.** Исходный read-only baseline 21.08.2026:
466 POI, из них 178 хотя бы с одним замечанием — 40 без координат, 25 без русского описания, 11 в `Review`
при уже готовом Approved-тексте, 8 остановок маршрутов без POI, 4 пары кандидатов на дубль или
`Parent POI`. Это историческая точка отсчёта; текущее состояние после 10e-E2 находится в § 13 и в каноне.
...
- **Массовый импорт и cron ждут канонического baseline.** Для записей без координат координатный гейт
  дедупа не срабатывает: аудит доказывает именно это — не появление дублей, а рост числа неопределённых
...
Текущий канон POI описан в `docs/poi-intake/README.md`, а безопасные маршруты изменений —
в `docs/poi-intake/agent-maintenance-guide.md`. Граница Airtable Intake Core и адаптеров порталов — в
`docs/poi-intake/portal-adapter-architecture.md`; план Japan Guide — в
`docs/poi-intake/portal-adapter-rollout-plan.md`. Механические нарушения останавливают
`check:canon`, `check:poi` и специализированные POI-тесты; отменённой информационной мерки
`poi:score` в проекте больше нет.
...
- **Канонизация Airtable (состояние 2026-08-26 после 10e-E2).** Связность Route Stops закрыта: 471 POI,
  64 остановки, активных остановок без `POI ID` нет; остаются два осознанных `snapshot_drift`. В рамках 10e
  записано 23 `japan-guide:*` и 5 `owner-curation:*` Source Key. Массовый импорт и cron по-прежнему ждут
  редакторских, координатных и идентификационных очередей; точные числа —
...
- **Сопоставление с Airtable: 918 из 1140 объектов в базе не найдены (2026-08-26).** 23 доказанные адресом
  связи записаны как `japan-guide:*` и при повторной раскладке перешли из `linkedByUrl` в `linkedByKey` без
  изменения пары. Кандидатов по имени 197 (решает человек), неоднозначных 2 (`Rinnoji Temple` в Никко и в
  Сэндае). Пять `owner-curation:*` из 10e-E2 к discovery-корпусу не относятся. Массовый импорт ждёт ручной
  очереди и канонизации остальных контуров.
- ~~**Публичная граница `applyCityTourStopOverrides` покрыта не полностью.**~~ **Закрыто 26.08.2026:**
  выбор раздела вынесен чистой `selectPhotoFallback` в `src/lib/city-tour-stop-merge.ts`, а сам фасад стал
  загружаемым голым Node через хук-резолвер `tests/support/alias-loader.mjs` (подключается `registerHooks`).
  Ни один production-файл ради теста не переписан: тест исполняет НАСТОЯЩУЮ `applyCityTourStopOverrides` с
  НАСТОЯЩИМ `route-stop-photos.generated.json`. Проверены три известных слага, неизвестный, пустой,
  отсутствующий, ключи из прототипа (`__proto__`, `constructor`, `toString`) и файл без `bySlug`; на уровне
  фасада — чтение реального файла, передача собственного `routeSlug`, неразрывный пробел в кодовом заголовке
  и приоритет фотографии из Airtable над подстраховкой. Мутации: 12/12, каждая на своём именованном
  утверждении.

  Урок, ради которого это записано. Первая версия мутации «фасад подставляет чужой слаг» убивалась СОСЕДНИМ
  утверждением, а не своим: разделы `bySlug` не пересекаются ни одним заголовком, поэтому и подмена слага, и
  отказ читать файл давали одинаковое «фото нет». Поймал это сторож `proves`, а не глаз. Случаи разведены
  заголовками, которые встречаются ровно в одном разделе («Гиндза» — только `day-one`, «Акихабара» — только
  `hidden-spots`), и порядок утверждений теперь часть доказательства, а не оформление.
- ~~**Операционной цели приёмки для `v3` нет.**~~ **Закрыто 25.08.2026 независимым повтором:** второй живой
  полный обход дал те же 208 / 150 / 1 / 58 / 1140 / 1293 и пустые каналы отказов; множества ключей,
  семантика и граф совпали. Изменились только байты свидетельств источника, что монитор отделил от
  структурного дрейфа. Canary остаётся `v1`/`v2`-only; для `v3` воротами служат контракт, tracked baseline и
  сравнение двух полных снимков.
- ~~**Скил `jj-db-dev` на диске отстаёт от накопленного опыта.**~~ **Закрыто 26.08.2026:** Claude- и
  Codex-версии обновлены общей L3-доктриной и уроками 10e-E2. Черновики `tmp/jj-db-dev-SKILL-r*.md` не
  применяются и источником правды не являются; действуют только `.claude/skills/jj-db-dev/` и
  `.agents/skills/jj-db-dev/`.
