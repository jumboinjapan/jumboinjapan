...
- **Живой полный обход `v3` не выполнялся (2026-08-24).** Граф коллекций реализован и проверен офлайн:
  синтетическая топология измеренного корпуса даёт 208 целей, 150 коллекций каталога, одну вложенную,
  1141 объект, 1141 запись, ноль отказов и 1294 обмена — те же числа, что дал живой обход `v2`. Но живой
  повторный обход по `v3` остаётся отдельным СЕТЕВЫМ гейтом после ревью: только он покажет, сколько на
  самом деле вложенных коллекций у источника. Числа `150 / 1 / 1141` до этого прогона — модель, а не
  измерение.
