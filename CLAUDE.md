...
- **PDF**: страница «Стоимость программы» (цена на человека с разбивкой, `drawPricingPage`) печатается только когда `route.pricing` заполнен и флажок включён; ошибки расчёта не роняют печать. HTML-превью `/admin/print/*` смету пока НЕ показывает — только PDF.
