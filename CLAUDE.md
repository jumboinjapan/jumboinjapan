...
- **PDF**: страница «Стоимость программы» (цена на человека с разбивкой + ремарки ставок, `drawPricingPage`) печатается только когда `route.pricing` заполнен и флажок включён; ошибки расчёта не роняют печать. HTML-превью `/admin/print/*` показывает смету ВСЕГДА (с пометкой, попадёт ли она в PDF). На каждой странице PDF — водяной знак jumboinjapan.com (`drawWatermark`).
