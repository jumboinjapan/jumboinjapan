...
_Last reviewed: 2026-08-21 (этап 10c закрыт: контракт discovery разведён на `v1`/`v2` с единой `VERSION_POLICY`, закрыта связность отказов, публичные строители доведены до строгости агрегата. Read-only аудит Airtable нашёл 178 POI с замечаниями — канонизация базы стала входным условием массового импорта и cron)._
