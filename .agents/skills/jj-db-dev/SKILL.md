...
- Before touching any lock anywhere under `.git`, first determine whether the intended operation needs it. Inspect nested locks too: `.git/index.lock` protects the index, `.git/HEAD.lock` protects `HEAD`, and `.git/objects/maintenance.lock` protects maintenance or garbage collection. An unrelated lock is not a reason to remove it; for example, index and `HEAD` locks do not need to be removed merely to push. If a lock actually blocks a write, establish its owner from the strongest available evidence; absence from `ps` is insufficient when the process view may be namespaced and unable to see host processes. Leave an unproven lock alone and ask the owner.
- Treat a mounted checkout as **Git-write-unsafe for the rest of the session** as soon as an observed Git command owned by this agent finishes but cannot remove its own lock or object temporary file (`EPERM`, `Operation not permitted`, or an equivalent cleanup failure). Do not probe this by creating anything under `.git`. After the condition is observed, use only read-only commands with `git --no-optional-locks`; do not run `add`, `commit`, `checkout`, `stash`, `merge`, `rebase`, `fetch`, `pull`, `gc`, `prune`, or `push` from that mount. Hand the exact intended Git transition to Codex or another host that can update the repository normally.
- Do not move or delete lock files or `tmp_obj_*` residues from the Git-write-unsafe mount, even when they appear to come from this agent. Report their exact paths and the observed command failure; let the capable host verify repository state and perform any cleanup. A successful workaround must not be used as permission to continue Git writes from the unsafe mount.
...
- A digest proves byte identity, not format suitability. After decoding and digest verification, bind the artifact's actual contract version to the version declared by its manifest and to the consumer's closed supported-version set before reading version-specific fields.
- When spelling itself determines rejection, perform the lexical check before `new URL`, decoding, `String(value)`, trimming, or any other normalizer. Accept the original input type and normalize only equivalences explicitly guaranteed by the contract.
- Keep a baseline manifest and its complete referenced artifact together in Git. Store separate digests for compressed bytes, decompressed bytes, and the semantic snapshot; none substitutes for another. Do not promise cross-environment gzip-body reproducibility, because deflate output depends on the zlib build; treat the accepted tracked bytes as immutable evidence.
