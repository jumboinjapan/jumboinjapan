...
;(async () => {
  ...
  // ── 3b. Уход с записи до конца загрузки не убивает её навсегда ───────
  /* POI-000371, 9 августа: клик по записи, уход на соседнюю до того, как
     подтянулись тексты, и возврат — карточка навсегда писала «не
     загрузились». Отменялась не только подписка, но и запись результата,
     а отметка «уже запрошено» оставалась. */
  {
    const slow = rows.find((b) => b !== target && b !== rows[0]) || rows[2]
    const slowName = slow.innerText.split('\n')[0]
    slow.click()
    await sleep(120)
    target.click()
    await sleep(3000)
    slow.click()
    await sleep(6000)
    const stuck = document.body.innerText.includes('не загрузились')
    ok('уход с записи до конца загрузки не ломает её', !stuck, stuck ? `«${slowName}» больше не грузится` : '')
    target.click()
    await sleep(2000)
  }
