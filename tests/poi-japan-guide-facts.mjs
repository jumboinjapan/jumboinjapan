...
test('EVIDENCE inert template is excluded without empty locators',()=>{const e=parseJapanGuideEvidence({...page,text:'<main><template><span>Hidden favourite</span></template><p>Visible article</p></main>'});assert(!e.blocks.some(b=>b.text.includes('Hidden favourite')));assert(e.blocks.every(b=>b.locator));assert(e.blocks.some(b=>b.text==='Visible article'))})
test('EVIDENCE official custom map retains query and strips key',()=>{const e=parseOfficialPageEvidence({...page,url:'https://tourism.example.org/spot/1',text:'<main><p>Place</p><detail-map-marker-component embed-url="//www.google.com/maps/embed/v1/place?key=secret&amp;q=32.5,128.5"></detail-map-marker-component></main>'},{sourceKey:'japan-guide:e4442',rootSelector:'main'});const b=e.blocks.find(b=>b.map);assert(b);assert.equal(b.map.query,'32.5,128.5');assert(!b.url.includes('secret'));assert(b.locator)})
...
const {readPoiGeographyDocument}=await import('../src/lib/poi-geography-document.ts')
const {readMatrixRecord}=await import('../scripts/poi-portals/lib/poi-matrix-catalog.mjs')
const mapRecords=new Function('normalizeWorkspaceCopyStatus','readPoiCategory','readPoiGeography','readMatrixRecord','readPoiGeographyDocument',`${executable};return mapPoiRecords`)(v=>String(v??''),readPoiCategory,readPoiGeography,readMatrixRecord,readPoiGeographyDocument)
test('PRIVACY public projection never includes internal Notes',()=>{const rows=[{id:'rec00000000000001',fields:{Notes:'PRIVATE_DOSSIER'}}];assert(!Object.hasOwn(mapRecords(rows,new Map())[0],'notes'));assert(!Object.hasOwn(mapRecords(rows,new Map())[0],'matrix'));assert.equal(mapRecords(rows,new Map(),true)[0].notes,'PRIVATE_DOSSIER')})
