...
// ── Пакет через конвейер: гейт видит уже созданные в этом же прогоне ─────
{
  ...
  // Соседство по координатам — не блокировка: у храмового комплекса
  // соседние постройки стоят в одной точке и остаются разными записями.
  check('сосед по координатам создан', results[3].outcome, 'created')
  check('но сосед отмечен в Notes', /РЯДОМ \(по координатам\)/.test(results[3].fields.Notes), true)
}
