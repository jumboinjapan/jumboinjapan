...
try{
  ...
  await test('CLI_READ_REPLAY_SAME_BUNDLE',async()=>{
    ...
    const out=path.join(dir,'read'),t=transport();await runHokkaidoCli(['read',selection,out],t)
    ...
    const run=JSON.parse(await readFile(path.join(out,'run.json'),'utf8'));assert.equal(run.readyForWrite,false);assert.equal(run.effects.airtable,0)
  })
