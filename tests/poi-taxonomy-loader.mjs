...
const REGISTRY_REL = 'config/poi-taxonomy.v4.json'
...
t('loader imports current and previous registries only', (loaderSource.match(/\bfrom '[^']+\.json'/g) ?? []).length, 3)
t(
  'импортируется именно реестр',
  /from '\.\.\/\.\.\/config\/poi-taxonomy\.v2\.json' with \{ type: 'json' \}/.test(loaderSource),
  true,
)
t('current registry version is fourth', tx.taxonomyVersion, 'poi-taxonomy/v4')
