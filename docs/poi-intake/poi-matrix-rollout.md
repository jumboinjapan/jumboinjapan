...
M1 прошёл независимую приёмку. [M3 — запись и адаптеры](poi-matrix-m3.md)
реализован. [Пилот M4](poi-matrix-m4.md) выполнен: 50 матриц в живой базе,
ошибок чтения 0; партиями до 100 заполнено 807 из 1171 карточек
([последняя приёмка](poi-matrix-enrichment-2026-09-19.md)). После [регистрации 36 POI Хоккайдо](hokkaido-route-results-2026-09-20.md) остаток M4 — 364 карточка;
весь этап не закрыт. По решению владельца от 19.09 сначала дополняем факты
и описания; матрицу заполняем отдельно небольшими партиями.
[M5 — подбор программы](poi-matrix-m5.md) реализован; заполнение оставшейся базы M4 продолжается отдельно.
...
## Целевой результат для владельца
...
|---|---|---|
| Реестр типов | `src/lib/poi-taxonomy.ts`, активный `config/poi-taxonomy.v6.json` | Один основной тип; четыре фасета не покрывают функции и условия |
| Чтение и фильтры | `src/lib/poi-category.ts`, `AdminOperationsConsole.tsx` | Матрица подключена к каталогу; конструктор и оставшиеся потребители — M5/M6 |
| Запись классификации | `poi-taxonomy-airtable.ts`, `classification-contract.mjs`, `poi-ingest.ts` | Общий Intake принимает проверенную матрицу; старые поля сохранены для совместимости |
| Дополнение старых мест | `poi-fact-sync.mjs`, команда `poi:sync` | Пишет матрицу общим sync; отдельный пакет matrix-only сохраняет остальные поля |
...
`poi-matrix/v1`. **Живое поле создано и проверено в пилоте M4 15.09.2026.**
Проверка предложений и чтение — M1, UI — M2, запись — M3.
...
|---|---|---|---|
| M1. Контракт и эталоны | Реестр слоёв, состояний, правил и хранения; 30–50 разных POI как эталоны | Гора/храм, замок/музей/сад, аквариум, общественное здание, комплекс, туристический транспорт и семейные ограничения представлены; нет обязательного выбора единственной функции | Принят независимо на 7ae185a; 40 примеров, семейный сценарий синтетический |
| M2. Общее чтение и интерфейс | Reader/validator и компактная проекция; галочки и подписи в админке, просмотр источников; совместимость со старой базой | Старая карточка видна, новая находится по нескольким функциям; одинаковые фильтры дают те же POI ID; телефон и клавиатура работают | Подключён к main и сайту; проверен |
| M3. Запись и все адаптеры | Расширить существующие Intake и sync; проверять предложение по фактам и схеме; импортировать новые свойства и дополнять известные | Сквозные тесты обоих путей, повтор без лишней записи, отказ по чужому факту, дрейфу и неподтверждённому исходу; понятный отчёт изменений | Реализован: общий Intake/sync, Japan Guide и Hokkaido; живая схема проверена в M4 |
| M4. Пилот и миграция базы | Пилот 50 карточек; затем оставшиеся из 270, затем остальная база партиями до 100 (пакеты writer до 25) | Отчёт old → new по каждому POI, независимая сверка записи, исход у каждого ID; нет потерь фактов, описаний, связей и статусов | Пилот и продолжение приняты: 807/1171; прежняя очередь оценки исчерпана; осталось 364 без матрицы, из них 36 новых POI Хоккайдо; два прежних родителя остаются без подходящего свойства; ограничения полноты — в отчётах M4 и последней партии |
| M5. Подбор программы и профиль | Общий поиск для конструктора и остановок; пожелания гостя → объяснимые фильтры/приоритеты; сохранённые наборы фильтров | Гид получает подходящую выборку, объяснение совпадений и неизвестных условий; добавление POI сохраняет связи, время, стоимость и ручные правки программы | Реализован: общий подбор в двух редакторах, профиль и наборы; [контракт и проверка](poi-matrix-m5.md) |
