...
**Админ-CRM** (редизайн по фидбеку владельца 2026-07-05: плитки/канбан заменены списком): `/admin/clients` — единый список от начала воронки к концу, сгруппированный секциями по `Stage`; сверху фильтры-чипы («В работе» = received+processed+discussing+agreed, затем каждая стадия с количеством кейсов). Строка: имя, тип тура, состав, даты, source, индикатор анкеты, «N дней в стадии» (внутри стадии дольше ждущие — выше), компактный select смены стадии; строка целиком кликабельна → client workshop. Без drag-n-drop (смена стадии — осмысленное действие, не сортировка).

Client workshop `/admin/clients/[prospectId]`: «профиль туриста» сверху (пустые поля — приглушённое «—»; анкета не заполнена → empty-state с кнопкой «Скопировать ссылку»), затем «Маршруты клиента», затем воронка (Stage + `Tour Type` + таймлайн) и контакт/заметки, внизу — **лог комментариев** (поле `Comments`, JSON-массив `[{at, text}]`, пишется только через `appendProspectComment`; отдельно от Notes, потому что Notes перезаписывается денормализацией опросника). Билдеры не множатся: workshop открывает существующий Multi-Day Builder с контекстом — `/admin/multi-day?client=recX` показывает баннер и автопривязывает сохранённый маршрут к `Linked Routes`, `&route=slug` автозагружает привязанный маршрут на редактирование. API `/api/admin/clients*` — через prospects.ts, за периметром.
