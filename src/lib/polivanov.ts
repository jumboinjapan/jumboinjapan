...
/**
 * Японские топонимы НЕ СКЛОНЯЮТСЯ (канон владельца 01.08.2026):
 * «в Сибуя», «от Гинза до Асакуса», «из Камакура».
 *
 * Исключения — Токио, Киото, Осака: склоняются повсеместно и в неизменяемом
 * виде режут глаз. Их здесь нет намеренно.
 *
 * Подтверждено замером 10.08.2026: владелец просмотрел 188 живых фраз базы
 * в виде «было → стало» и вернул НОЛЬ. Формулировка его: несклоняемый
 * вариант работает во всех случаях, склоняемый — нестабильно. Это довод
 * сильнее ссылки на решение: правило проверено на корпусе, а не объявлено.
 */
export const INDECLINABLE_TOPONYMS: readonly string[] = [
  'Хигасияма', 'Сибуя', 'Гинза', 'Асакуса', 'Камакура', 'Нара', 'Мацусима',
  'Канадзава', 'Сэндай', 'Хиросима', 'Биэй', 'Коясан', 'Никко', 'Хаконе',
  'Кавагутико', 'Тоёсу', 'Отару', 'Саппоро', 'Кусиро', 'Химэдзи', 'Нагасаки',
]

/**
 * Падежные формы, которых быть НЕ ДОЛЖНО. Выводятся из окончания, а не
 * перечисляются: список из двухсот форм разошёлся бы с базой в тот день,
 * когда в неё добавят город. Правило «выводить, а не перечислять» здесь
 * то же, что и у канонических написаний выше.
 */
export function declinedForms(base: string): string[] {
  const stem = base.slice(0, -1)
  const last = base.slice(-1)
  if (last === 'а') return [stem + 'ы', stem + 'е', stem + 'у', stem + 'ой']
  if (last === 'я') return [stem + 'и', stem + 'е', stem + 'ю', stem + 'ей']
  if (last === 'й') return [stem + 'я', stem + 'е', stem + 'ю', stem + 'ем']
  if (last === 'н') return [base + 'а', base + 'е', base + 'у', base + 'ом']
  // На -о, -у, -и, -э русский и так не склоняет: Никко, Тоёсу, Нагасаки.
  return []
}

/** Все запрещённые падежные формы разом — для проверки текстов. */
export const DECLINED_TOPONYM_FORMS: ReadonlyArray<readonly [wrong: string, right: string]> =
  INDECLINABLE_TOPONYMS.flatMap((base) => declinedForms(base).map((f) => [f, base] as const))
