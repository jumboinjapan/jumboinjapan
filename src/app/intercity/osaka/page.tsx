...
export default async function OsakaPage() {
  const [routeStopRecords, pois] = await Promise.all([
    getIntercityRouteStopsCached('intercity/osaka'),
    getPoisByCityCached('osaka'),
  ])
  ...
  return (
      ...
      <section className="border-t border-[var(--border)] bg-[var(--bg-warm)] px-4 py-12 md:px-6 md:py-16">
        <div className="mx-auto w-full max-w-6xl space-y-10 md:space-y-14">
          ...
          <section className="space-y-6 md:space-y-8">
            ...
            <p className="text-[13px] text-[var(--text-muted)]">До Осаки едут по железной дороге; вечерняя часть программы раскрывается лучше, если вы живёте в городе.</p>
    ...
    <RouteFaq slug="intercity/osaka" />
    <JournalMentions
      routeSlug="intercity/osaka"
      poiIds={timelineStops.map((s) => s.poiId).filter((id): id is string => Boolean(id))}
      locationNames={[...timelineStops.map((s) => s.title), 'Осака']}
