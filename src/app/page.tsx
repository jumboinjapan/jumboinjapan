...
const approach = typoDeep({
  ...
  columnOne: [
    // Абзац подрезан на строку сознательно: так колонки становятся 6 и 5 строк
    // вместо 7 и 5 и не расходятся по низу. Удлинять его обратно — значит
    // вернуть перекос, лечить который придётся вёрсткой.
    "Япония часто остаётся понятой лишь наполовину: насыщенное путешествие превращается в набор красивых кадров, если не хватает контекста, ритма и оптики.",
