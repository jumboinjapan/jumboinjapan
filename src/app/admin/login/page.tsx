  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl items-center px-4 py-10 md:px-6 md:py-12">
      <div className="grid w-full gap-6 xl:grid-cols-[minmax(0,1.1fr)_30rem]">
        <section className="relative overflow-hidden rounded-[2rem] border border-[var(--adm-border)] bg-[#07111f]/95 px-6 py-7 shadow-[0_30px_80px_rgba(3,8,20,0.45)] md:px-8 md:py-8">
          ...
          <div className="relative space-y-8">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="max-w-2xl text-sm leading-7 text-[var(--adm-text-2)] md:text-[15px]">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <FeatureTile
                icon={<LockKeyhole className="size-4 text-[var(--adm-on-accent)]" />}
                title="Private by default"
                body="Noindex and robots protection stay in place across admin routes."
              />
              <FeatureTile
                icon={<ShieldCheck className="size-4 text-[var(--adm-on-accent)]" />}
                title="Approved accounts"
                body="Google allowlisting governs who can enter the workspace."
              />
              <FeatureTile
                icon={<Sparkles className="size-4 text-[var(--adm-on-accent)]" />}
                title="Operational focus"
                body="Built for internal editing, approvals, and future system modules."
              />
            </div>

            <div className="rounded-[1.6rem] border border-[var(--adm-border)] bg-[var(--adm-active)] p-4 text-sm leading-6 text-[var(--adm-text-2)]">
              Main routes: <Link href={getAdminLoginPath()} className="font-medium text-[var(--adm-text)] underline underline-offset-4">/admin/login</Link>,{' '}
              <Link href="/admin" className="font-medium text-[var(--adm-text)] underline underline-offset-4">/admin</Link>, and{' '}
              <Link href="/admin/seo-llm" className="font-medium text-[var(--adm-text)] underline underline-offset-4">/admin/seo-llm</Link>.
            </div>
          </div>
        </section>

        <section className="rounded-[2rem] border border-[var(--adm-border)] bg-[#081220]/92 p-6 shadow-[0_24px_60px_rgba(3,8,20,0.35)] md:p-7">
          <div className="space-y-5">
            <div className="space-y-2">
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--adm-text-3)]">Authentication</p>
              <h2 className="text-2xl font-semibold tracking-[-0.03em] text-[var(--adm-text)]">Sign in to continue</h2>
            ...
            {googleReady ? (
              ...
              <div className="rounded-[1.4rem] border border-dashed border-[var(--adm-border)] bg-[var(--adm-hover)] p-4 text-sm leading-6 text-[var(--adm-text-2)]">
                Google admin auth is not fully configured yet. Add the required env vars before using Google sign-in.
              </div>
            )}

            {basicFallback ? (
              <div className="rounded-[1.4rem] border border-[var(--adm-border)] bg-[var(--adm-hover)] p-4 text-sm leading-6 text-[var(--adm-text-2)]">
                Basic Auth fallback is enabled as a temporary safety net. Once Google login is verified, it can be turned off.
              </div>
            ) : null}

            <div className="rounded-[1.4rem] border border-[var(--adm-border)] bg-[#0b1728]/88 p-4">
              <div className="text-[11px] uppercase tracking-[0.2em] text-[var(--adm-text-3)]">Access notes</div>
              <ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--adm-text-2)]">
...
function FeatureTile({
  ...
  return (
    <div className="rounded-[1.4rem] border border-[var(--adm-border)] bg-[var(--adm-active)] p-4 backdrop-blur-sm">
      <div className="flex size-10 items-center justify-center rounded-2xl border border-[var(--adm-border)] bg-[var(--adm-active)]">{icon}</div>
      <div className="mt-3 text-sm font-medium text-[var(--adm-text)]">{title}</div>
      <p className="mt-1 text-sm leading-6 text-[var(--adm-text-2)]">{body}</p>
    </div>
  )
}
