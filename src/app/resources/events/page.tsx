...
export default async function ResourceEventsPage({ searchParams }: EventsPageProps) {
  ...
  return (
    <ResourcesSectionShell
      ...
      description="Редакционный календарь выставок, фестивалей, концертов и сезонных событий в Японии внутри общей базы ресурсов для поездки."
      guidanceTitle="Как читать live-раздел"
      guidanceItems={[
        {
          title: 'Смотрите на окно дат',
          description: 'Это временные записи. Проверяйте период проведения перед тем, как встраивать событие в уже собранный маршрут.',
        },
        {
          title: 'Используйте как дополняющий слой',
          description: 'Сначала соберите города, ночёвки и логистику, а потом накладывайте выставки, концерты и сезонные поводы.',
        },
        {
          title: 'Сверяйте официальный источник',
          description: 'Карточки помогают быстро отобрать идеи, но финальную проверку дат, билетов и правил посещения лучше делать на сайте площадки.',
        },
      ]}
    >
        ...
        <div className="flex min-w-max flex-nowrap gap-2 pb-1">
          ...
          {eventCategories.map((category) => {
            ...
            return (
              <Link
                key={category}
                href={`/resources/events?category=${category}`}
                className={`inline-flex min-h-11 shrink-0 items-center justify-center px-5 py-2 text-sm font-medium tracking-wide uppercase transition-colors ${
                  isActive
                    ? 'bg-[var(--text)] text-[var(--bg)]'
                    : 'border border-[var(--text)] text-[var(--text)] hover:bg-[var(--text)] hover:text-[var(--bg)]'
            ...
            <article key={event.id} className="border border-[var(--border)] bg-white p-6 md:p-7">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div className="space-y-2">
                  <p className="text-xs font-medium tracking-wide text-[var(--text-muted)] uppercase">{categoryLabels[event.category]}</p>
              ...
              <div className="mt-5 grid gap-2 text-sm text-[var(--text)] md:grid-cols-2">
                <p>
                  <span className="text-[var(--text-muted)]">Даты:</span> {formatEventDate(event.dateStart)} — {formatEventDate(event.dateEnd)}
                </p>
                <p>
