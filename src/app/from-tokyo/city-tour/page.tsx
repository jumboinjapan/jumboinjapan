...
const programs = [
  {
    ...
    description: "Классический маршрут по главным точкам города — от Гинзы до Сибуи. То, что стоит увидеть в Токио первым делом, но без туристического конвейера.",
    duration: "6–8 часов",
...
const transportOptions = [
  {
    ...
    description: "В маршруте, не считая дороги от отеля до места старта и обратного возвращения в конце дня, предусмотрен всего один переезд на общественном транспорте. Это удобная возможность заодно увидеть, как работает токийское метро.",
    href: "/from-tokyo/city-tour/public",
  },
  {
    title: "Такси",
    description: "В Токио хорошо развита служба такси, и пользоваться ей можно не только через приложение. Это удобный вариант для коротких переездов по городу. Если хочется большего комфорта, рекомендую Uber Black Van.",
    href: "/from-tokyo/city-tour/public",
  },
  {
    title: "Лимузин сервис",
    description: "Для групп, которым важен максимальный комфорт — индивидуальный транспорт с водителем на целый день. Минивэн заберёт вас в условленных точках маршрута и при необходимости доставит покупки в отель.",
            ...
            {transportOptions.map((option) => (
              <article key={option.href} className="group flex h-full flex-col overflow-hidden">
                ...
                <div className="mt-5 flex h-full flex-col gap-3">
                  <h3 className="font-sans font-semibold text-lg tracking-tight">{option.title}</h3>
                  <p className="font-sans text-sm leading-[1.7] text-[var(--text-muted)]">{option.description}</p>
                  <Link
                    href={option.href}
