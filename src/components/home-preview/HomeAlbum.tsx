...
export function HomeAlbum({ preview = false }: { preview?: boolean }) {
  ...
  return <div className={`${styles.preview} ${styles.album}`}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeTourSchema([
      { '@context': 'https://schema.org', '@type': 'WebPage', url: pageUrl, name: 'Частный гид по Японии — Эдуард Ревидович', inLanguage: 'ru', about: guideRef },
      faqSchema,
    ]) }} />
    {preview ? <PreviewSwitch active="b" /> : null}
