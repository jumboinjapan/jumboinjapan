        ...
        <div className="mx-auto w-full max-w-6xl space-y-14 md:space-y-16">
          <section className="grid gap-8 md:grid-cols-[minmax(0,1.3fr)_minmax(280px,0.9fr)] md:items-start">
            ...
            <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 md:p-6">
              <p className="text-label font-medium uppercase tracking-[0.18em] text-[var(--accent)]">Что здесь важно</p>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              {overnights.map((row) => (
                <div key={`${journey.slug}-${row.city}`} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-5 py-4">
                  <p className="text-label font-medium uppercase tracking-[0.18em] text-[var(--accent)]">Ночёвка</p>
                  <p className="mt-2 text-body font-medium tracking-[-0.01em] text-[var(--text)]">{row.city}</p>
                  <p className="mt-1 text-body-sm font-light leading-[1.7] text-[var(--text-muted)]">{row.nights} ночи</p>
                </div>
              ))}
            </div>
          </section>
