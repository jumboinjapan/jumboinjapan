      ...
      <header className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)] px-4 py-3 text-right">
            ...
            <p className="mt-2 text-body-sm font-light leading-[1.7] text-[var(--text-muted)]">{journey.geography}</p>
          </div>
        </div>
        <p className="max-w-3xl text-body-sm font-light leading-[1.8] text-[var(--text-muted)]">{journey.rhythm}</p>
      </header>
        ...
        {journey.days.map((day, dayIndex) => (
          ...
          >
            <summary className="flex cursor-pointer list-none items-start justify-between gap-4 px-5 py-4 marker:content-none">
              <div className="space-y-2">
                <p className="text-label font-medium uppercase tracking-[0.18em] text-[var(--accent)]">День {day.day}</p>
                <h4 className="font-sans text-lead font-medium leading-[1.3] tracking-[-0.01em] text-[var(--text)]">{day.title}</h4>
                <p className="max-w-2xl text-body-sm font-light leading-[1.75] text-[var(--text-muted)]">{day.summary}</p>
              </div>
              <div className="flex shrink-0 items-start gap-3 text-right">
                <div className="space-y-1">
                  <p className="text-label font-medium uppercase tracking-[0.16em] text-[var(--accent)]">Ночёвка</p>
                  <p className="text-meta font-light leading-[1.6] text-[var(--text-muted)]">{day.overnightCity}</p>
                </div>
            ...
            <div className="space-y-4 border-t border-[var(--border)] px-5 py-4">
              {day.transfers?.map((transfer) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-2 text-meta font-medium tracking-[0.01em] text-[var(--text)]">
                  ...
                  {transfer.note && <p className="mt-2 text-meta font-light leading-[1.7] text-[var(--text-muted)]">{transfer.note}</p>}
                </div>
              ))}
                    ...
                    >
                      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:content-none">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-label font-medium uppercase tracking-[0.16em] text-[var(--accent)]">Место</p>
                            <p className="text-body-sm font-medium leading-[1.5] text-[var(--text)]">
                        ...
                        <ul className="space-y-2">
                          {city.pois.map((poi) => (
                            <li key={`${city.name}-${poi.name}`} className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-body-sm font-light leading-[1.75] text-[var(--text-muted)]">{poi.name}</p>
                                {poi.note && <p className="mt-1 text-meta font-light leading-[1.7] text-[var(--text-muted)]">{poi.note}</p>}
                              </div>
                            </li>
                          ))}
                        </ul>
              ...
              <div className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-label font-medium uppercase tracking-[0.16em] text-[var(--accent)]">Ночёвка</p>
                    <p className="text-body-sm font-light leading-[1.7] text-[var(--text-muted)]">{day.overnightCity}</p>
                  </div>
                </div>
              </div>
            </div>
